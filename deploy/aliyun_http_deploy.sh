...
# Alethea HTTP版本阿里云部署脚本
# 适用于没有ICP备案的情况，仅部署HTTP版本
...
deploy_application() {
    ...
    if [ ! -d "$APP_DIR" ]; then
        ...
        log_info "请先运行代码上传脚本: ./deploy/upload_to_server.sh YOUR_SERVER_IP"
        exit 1
    fi
...
EOF
    ...
    log_warning "请编辑 ${APP_DIR}/.env 文件，填入实际的API密钥"
}
...
[Unit]
Description=Alethea Web Application (HTTP)
...
# 配置Nginx (HTTP版本)
configure_nginx() {
    log_info "配置Nginx (HTTP版本)..."
    
    # 使用HTTP版本的Nginx配置
    cp ${APP_DIR}/deploy/nginx_http.conf /etc/nginx/sites-available/alethea
...
configure_firewall() {
    ...
    if command -v ufw &> /dev/null; then
        ...
        ufw allow 80/tcp
        ufw allow 8083/tcp
        log_info "已开放端口: 22 (SSH), 80 (HTTP), 8083 (应用)"
...
if [ "$HTTP_STATUS" != "200" ]; then
    echo "$(date): HTTP health check failed (status: $HTTP_STATUS), restarting services..." >> $LOG_FILE
...
show_deployment_info() {
    log_success "=== Alethea HTTP版本部署完成 ==="
    echo
    log_info "网站地址 (HTTP):"
    echo "  - http://${DOMAIN}"
    echo "  - http://${WWW_DOMAIN}"
    echo
    log_warning "注意: 当前为HTTP版本，没有SSL加密"
    echo
    ...
    echo "  2. 当前为HTTP版本，适用于没有ICP备案的情况"
    echo "  3. 如果后续获得ICP备案，可以升级到HTTPS版本"
    echo "  4. 建议配置阿里云OSS用于文件存储"
    echo
    log_info "升级到HTTPS版本:"
    echo "  1. 获得ICP备案后"
    echo "  2. 运行: certbot --nginx -d ${DOMAIN} -d ${WWW_DOMAIN}"
    echo "  3. 或使用原始的 aliyun_auto_deploy.sh 脚本"
    echo
}

# 主函数
main() {
    log_info "开始部署Alethea HTTP版本到阿里云..."
    log_info "域名: ${DOMAIN}, ${WWW_DOMAIN} (HTTP)"
    echo
    ...
    log_success "HTTP版本部署完成！请访问 http://${DOMAIN} 查看网站"
}
