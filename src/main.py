...
from models.llm_models_optimized import initialize_optimized_llm_providers
...
# Load configuration
def load_config():
    """Load configuration from environment or config file - 优化版配置"""
    config = {
        # 国内AI服务（优先级最高）
        'deepseek': {
        ...
        'ali_qwen': {
            ...
            'api_url': os.getenv('ALI_QWEN_API_URL', 'https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions'),
            'default_model': os.getenv('ALI_QWEN_DEFAULT_MODEL', 'qwen-plus')
        },
        'baidu_wenxin': {
            'api_key': os.getenv('BAIDU_WENXIN_API_KEY', ''),
            'secret_key': os.getenv('BAIDU_WENXIN_SECRET_KEY', ''),
            'default_model': os.getenv('BAIDU_WENXIN_DEFAULT_MODEL', 'ERNIE-4.0-8K')
        },
        'zhipu_ai': {
            'api_key': os.getenv('ZHIPU_AI_API_KEY', ''),
            'default_model': os.getenv('ZHIPU_AI_DEFAULT_MODEL', 'glm-4')
        },
        'kimi': {
        ...
        'claude': {
            ...
            'default_model': os.getenv('CLAUDE_DEFAULT_MODEL', 'claude-3-opus'),
...
def initialize_app():
    ...
    # Initialize optimized LLM providers
    initialize_optimized_llm_providers(config)
